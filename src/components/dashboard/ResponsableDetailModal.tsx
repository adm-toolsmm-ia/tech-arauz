...
export function ResponsableDetailModal({
  ...
  const [activeTab, setActiveTab] = useState<'performance' | 'comparison' | 'insights'>(
    'performance',
  );
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.altKey) && (e.key === '[' || e.key === 'ArrowLeft')) {
        e.preventDefault();
        const tabs: Array<'performance' | 'comparison' | 'insights'> = [
          'performance',
          'comparison',
          'insights',
        ];
  ...
  const performanceLabel =
    performanceDelta > 0 ? `+${performanceDelta}% vs média` : `${performanceDelta}% vs média`;

  const performanceColor =
    performanceDelta > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400';
        ...
        <DialogHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Análise detalhada de performance</p>
            </div>
            ...
            {activeTab === 'performance' && (
              <>
                ...
                <Card>
                  ...
                  <CardContent>
                    <DetailMetricsChart person={person} dateRange={dateRange} />
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <div className="bg-muted/30 flex items-start gap-3 rounded-lg border border-muted p-3">
                        ...
                        <div>
                          <p className="text-sm font-medium">
                            {efficiency > 70
                              ...
                              : efficiency > 50
                                ? '📈 Taxa de conclusão acima da média'
                                : '⚠️ Taxa de conclusão abaixo do esperado'}
                          </p>
                      ...
                      <div className="bg-muted/30 flex items-start gap-3 rounded-lg border border-muted p-3">
                        ...
                        <div>
                          <p className="text-sm font-medium">
                            {person.average_duration_days < 15
                              ? '⚡ Ritmo acelerado'
                              : person.average_duration_days < 30
                                ? '✓ Ritmo normal'
                                : '🐢 Ritmo lento'}
                          </p>
                          <p className="mt-0.5 text-xs text-muted-foreground">
                      ...
                      <div className="bg-muted/30 flex items-start gap-3 rounded-lg border border-muted p-3">
                        ...
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-muted-foreground">
                            {movementsPerDay} movimentações por dia —{' '}
                            {movementsPerDayValue > 2 ? 'muito ativo' : 'atividade moderada'}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            {activeTab === 'comparison' && (
              <>
                <Card>
                  ...
                  <CardContent>
                    <ComparativeChart person={person} allData={allData} dateRange={dateRange} />
                  </CardContent>
                </Card>
              </>
            )}
