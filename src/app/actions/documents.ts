...
export async function getDocumentBySlugAction(slug: string): Promise<ActionResult<DocumentRecord>> {
  const ctx = await getAuthContext();
  if (!ctx.ok) return { success: false, message: ctx.error };
...
export async function createDocumentAction(input: {
  ...
  if (!['admin', 'manager'].includes(profile.role)) {
    return {
      success: false,
      message: 'Sem permissão. Apenas admin/manager podem criar documentos.',
    };
  }
  ...
  if (input.content_md && input.content_md.length > CONTENT_MAX_LENGTH) {
    return {
      success: false,
      message: `Conteúdo excede o limite de ${CONTENT_MAX_LENGTH / 1000}k caracteres.`,
    };
  }
...
export async function updateDocumentAction(
  ...
  if (!['admin', 'manager'].includes(profile.role)) {
    return {
      success: false,
      message: 'Sem permissão. Apenas admin/manager podem editar documentos.',
    };
  }
