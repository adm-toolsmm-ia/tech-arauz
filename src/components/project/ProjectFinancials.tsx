...
export function ProjectFinancials({ projectId }: ProjectFinancialsProps) {
  const financials = useMemo(
    () => generateProjectFinancials(projectId),
    [projectId]
  );
      ...
      <div className="grid gap-4 grid-cols-3">
        <Card className="animate-scale-in">
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold mt-1">
              {formatCurrency(financials.budgeted)}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="animate-scale-in" style={{ animationDelay: '50ms' }}>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold mt-1">
              {formatCurrency(financials.realized)}
            </p>
          ...
          <div className="h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Bar dataKey="Orcado" fill="hsl(var(--muted-foreground))" opacity={0.5} radius={[0, 4, 4, 0]} />
                <Bar dataKey="Realizado" radius={[0, 4, 4, 0]}>
                  {chartData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
                        entry.Realizado <= entry.Orcado
                          ? 'hsl(142, 71%, 45%)'
                          : 'hsl(0, 84%, 60%)'
                      }
                    />
