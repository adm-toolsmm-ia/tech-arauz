...
export function DocumentacaoContent({ user, profile }: DocumentacaoContentProps) {
  ...
  const handleSave = React.useCallback(async () => {
    ...
    if (result.success && result.data) {
      setDocuments((prev) => prev.map((d) => (d.id === activeDoc.id ? result.data! : d)));
    ...
    <div className="flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden" style={{ height: 'calc(100vh - 80px)' }}>
        ...
        <aside className="w-72 flex-shrink-0 overflow-y-auto border-r bg-muted/30 p-4">
          ...
          {canEdit && (
            <Button variant="outline" size="sm" className="mb-4 w-full" onClick={startCreating}>
          ...
          {loading ? (
            ...
            <nav className="space-y-4">
              {CATEGORY_ORDER.map((cat) => {
                ...
                return (
                  <div key={cat}>
                    <h3 className="mb-1.5 flex items-center gap-1.5 px-1 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            ...
            <div className="mx-auto max-w-4xl p-6">
              ...
              <div className="mb-6 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {isEditing ? (
                    ...
                    <>
                      {activeDoc && (
                        <PdfExportButton contentRef={contentRef} title={activeDoc.title} />
                      )}
                      {canEdit && (
                        <>
              ...
              {isEditing ? (
                <div className="space-y-4">
                  ...
                  <div className="grid min-h-[60vh] grid-cols-2 gap-4">
                    {/* Editor pane */}
                    <div className="flex flex-col">
                      <div className="mb-1 flex items-center gap-1.5 border-b px-2 py-1 text-xs text-muted-foreground">
                ...
                <div ref={contentRef}>
                  {activeDoc && (
                    <>
                      <h1 className="mb-6 border-b border-border pb-3 text-2xl font-bold">
                        {activeDoc.title}
                      </h1>
                      <MarkdownViewer content={activeDoc.content_md} onTocGenerated={setTocItems} />
                    </>
                  )}
                </div>
              )}
        ...
        {activeDoc && !isEditing && tocItems.length > 0 && (
          <aside className="hidden w-56 flex-shrink-0 overflow-y-auto border-l p-4 xl:block">
