...
export function buildImpactMatrixData(projects: Array<any>): ImpactDataPoint[] {
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="rounded border bg-background p-2 text-sm shadow">
        ...
        {data.is_special && (
          <p className="mt-1 text-xs font-medium text-amber-500">⚠️ Importância Especial</p>
        )}
      </div>
    );
  }
  ...
  return (
    <Card className="flex h-full flex-col">
      ...
      <CardContent className="flex-1">
        <div
          role="region"
          aria-label="Gráfico de Matriz de Esforço x Impacto"
                ...
                <Scatter name="Projetos" data={plotData} fill="hsl(var(--primary))">
                  {plotData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
                        entry.impact_score >= 2.5 && entry.complexity_score <= 1.5
                          ? 'hsl(var(--chart-2))' // Quick Wins (Verde)
                          : entry.impact_score >= 2.5 && entry.complexity_score >= 2.5
                            ? 'hsl(var(--primary))' // Major Projects (Azul)
                            : entry.impact_score <= 1.5 && entry.complexity_score >= 2.5
                              ? 'hsl(var(--destructive))' // Time Wasters (Vermelho)
