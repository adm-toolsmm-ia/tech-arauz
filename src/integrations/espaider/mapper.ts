 ...
 */
function getExtras(campos: CampoEspaider[], camposMapeados: string[]): Record<string, string> {
...
export function mapearCronograma(registro: RegistroEspaider): CronogramaMapeado {
  ...
  return {
    ...
    data_fim: parseData(
      getCampoValor(campos, 'DATACONCLUSAO') || getCampoValor(campos, 'DATAPRAZO'),
    ),
 ...
 */
const CAMPOS_ORCAMENTO = ['IDREGISTROPAI', 'VALOR', 'FORNECEDOR', 'DATACOTACAO'];
 ...
 */
const CAMPOS_APROVADOR = ['IDREGISTROPAI', 'TIPO', 'RESPONSAVEL', 'PONTOSATENCAO'];
