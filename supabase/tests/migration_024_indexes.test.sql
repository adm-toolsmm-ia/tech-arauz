...
-- Test 1.1: Verify idx_integration_logs_tenant_created exists
SELECT
  indexname,
  tablename,
  indexdef
FROM pg_indexes
WHERE indexname = 'idx_integration_logs_tenant_created'
  AND schemaname = 'public';
-- Expected: 1 row (index exists)

-- Test 1.2: Verify idx_schedules_project_deliverable exists
SELECT
  indexname,
  tablename,
  indexdef
FROM pg_indexes
WHERE indexname = 'idx_schedules_project_deliverable'
  AND schemaname = 'public';
-- Expected: 1 row (index exists)
...
-- Test 2.1: Verify idx_integration_logs_tenant_created column composition
SELECT
  attname,
  attnum
FROM pg_index
JOIN pg_attribute ON pg_attribute.attrelid = pg_index.indrelid
WHERE pg_index.indexrelname = 'idx_integration_logs_tenant_created'
ORDER BY attnum;
-- Expected: 2 columns (tenant_id, created_at)

-- Test 2.2: Verify idx_schedules_project_deliverable column composition
SELECT
  attname,
  attnum
FROM pg_index
JOIN pg_attribute ON pg_attribute.attrelid = pg_index.indrelid
WHERE pg_index.indexrelname = 'idx_schedules_project_deliverable'
ORDER BY attnum;
-- Expected: 2 columns (project_id, deliverable_id)
...
-- Test 3.1: Verify no data loss or corruption on integration_logs
SELECT
...
-- Test 3.2: Verify no data loss on schedules
SELECT
  COUNT(*) as total_records,
  COUNT(DISTINCT project_id) as unique_projects,
  COUNT(DISTINCT deliverable_id) as unique_deliverables
FROM schedules;
...
-- Test 4.1: Verify application queries still work (integration_logs)
SELECT
  id,
  integration_type,
  status,
...
-- Test 4.2: Verify application queries still work (schedules)
SELECT
  id,
  project_id,
  deliverable_id,
...
-- Test 5.1: Verify idx_integration_logs_tenant_created is being used
...
-- Test 5.2: Verify idx_schedules_project_deliverable is being used
EXPLAIN (ANALYZE, BUFFERS)
SELECT id, deliverable_id, scheduled_date
FROM schedules
...
-- DROP INDEX CONCURRENTLY IF EXISTS idx_integration_logs_tenant_created;
...
-- Test 7.1: Measure query performance on integration_logs
\timing on
...
-- Test 7.2: Measure query performance on schedules
SELECT COUNT(*) as record_count
FROM schedules
WHERE project_id IS NOT NULL
GROUP BY project_id;
