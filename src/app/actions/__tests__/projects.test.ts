...
describe('fetchProjectsWithFiltersAction', () => {
  ...
  it('returns projects without filters', async () => {
    ...
    mockFrom.mockReturnValueOnce(profileChain).mockReturnValueOnce(projectsChain);
...
describe('updateProjectStatusAction', () => {
  ...
  it('blocks viewer role from updating status', async () => {
    ...
    const chain = buildChain({
      single: vi
        .fn()
        .mockResolvedValue({ data: { tenant_id: 'tenant-1', role: 'viewer' }, error: null }),
    });
    mockFrom.mockReturnValue(chain);
  ...
  it('updates status successfully and revalidates paths', async () => {
    ...
    mockFrom.mockReturnValueOnce(profileChain).mockReturnValueOnce(updateChain);
