...
export function AIInsightsPanel({
  ...
  const insights = useMemo(() => {
    ...
    const teamAvgCompletion = allData.length > 0
      ? Math.round(
        (allData.reduce((sum, d) => (sum + d.projects_completed / d.total_movements), 0) / allData.length) * 100
      )
      : 0;

    const personCompletion = person.total_movements > 0
  ...
  const predictionTrend = useMemo(() => {
    const personCompletion = person.total_movements > 0
      ? (person.projects_completed / person.total_movements) * 100
      : 0;
    const teamAvgCompletion = allData.length > 0
      ? (allData.reduce((sum, d) => sum + d.projects_completed / d.total_movements, 0) / allData.length) * 100
      : 0;
    ...
    <div className="space-y-4">
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <p className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{predictionTrend}</p>
            </div>
            <div>
        ...
        {insights.length > 0 ? (
          insights.map((insight, idx) => (
            ...
            >
              ...
              <CardContent>
                <div className="flex items-center gap-2 p-2 bg-background rounded border border-muted">
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm text-muted-foreground text-center">
                No specific insights available at this time. Performance metrics are within expected ranges.
              </p>
            </CardContent>
          </Card>
        )}
      ...
      <div className="p-3 bg-slate-50 dark:bg-slate-950/20 rounded-lg border border-slate-200 dark:border-slate-800">
        <p className="text-xs text-muted-foreground">
          💡 <strong>AI-Powered Insights:</strong> These recommendations are generated from historical performance data and team comparisons. Use them as decision support alongside domain expertise and contextual knowledge.
        </p>
      </div>
    </div>
  );
